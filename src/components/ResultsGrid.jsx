        ...
        <div className="animate-fade-in-up bg-surface-800/80 backdrop-blur-xl border border-surface-600/50 rounded-2xl p-5">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-4 md:gap-6">
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">{t('results.images')}</p>
                <p className="text-white font-bold text-lg">{completedImages.length}</p>
              </div>
              <div className="w-px h-8 bg-surface-600 hidden sm:block" />
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">{t('results.original')}</p>
              ...
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">{t('results.compressed')}</p>
                <p className="text-fab-400 font-bold">{formatFileSize(totalCompressed)}</p>
              </div>
              <div className="w-px h-8 bg-surface-600 hidden sm:block" />
              <div>
                <p className="text-gray-500 text-xs uppercase tracking-wider mb-1">{t('results.savings')}</p>
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              >
                ...
                {completedImages.length > 1 ? t('results.downloadZip') : t('results.download')}
              </button>
            </div>
          </div>
        </div>
